...
uint32_t created_req = 0;
uint32_t completed_req=0;
//uint32_t freed_req=0;
char load_signal=0;
vec_request *jy_ureq2vec_req(char* request_raw) {
	...
	uint32_t zip_max = 201326592;
	uint32_t zip_32 = 41943039;
	//printf("request: %s\n", request_raw);
	char* load_sig = "LOAD";	
...
bool jeeyun_end_req(request *const req) {
	...
	if (preq->size == preq->done_cnt) {
		...
		printf("size: %d, done: %d\n", preq->size, preq->done_cnt);
	}
